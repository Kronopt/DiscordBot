...
class Funny(Cog):
    ...
    def __init__(self, bot):
        ...
        self.eightball_answers = ["It is certain", "It is decidedly so", "Without a doubt",
    ...
    async def command_eightball(self, context, *args):
        ...
        await context.send('`' + context.message.clean_content + '`: ' +
                           self.eightball_answers[answer] + ' ' + emoji)
    ...
    async def command_joke(self, context):
        ...
        # These errors are all caught on DiscordBot.py on_command_error, but I wanted to be able to
        # catch a first error here so that I could switch apis on the fly in case one of them stops
        # working.
        # As it stands now, a first error should be caught here, then the api is switched and if
        # another error occurs, it should be caught in DiscordBot.py.
        ...
        try:
            ...
            await self.send_joke(context, api, joke)
        else:
    ...
    async def poop_on_error(self, error, context):
        bot_message = '`%s%s` takes no arguments or 1 positive number.' % (context.prefix,
    ...
    async def joke_on_error(self, error, context):
        ...
        if (isinstance(error, commands.CommandInvokeError) and
            ...
            await context.send(bot_message_beckett_error)
